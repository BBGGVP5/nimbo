...
- Xray обновлён до **26.9.9**. Загрузка и выбор ядра проверяются по закреплённым контрольным суммам; старый кэш не подменяет новую версию.
...
- **Nimbo Ping**: отдельный HTTP GET через каждый сервер подписки, без переключения текущего VPN. Метод стоит первым и используется по умолчанию, если свой способ ещё не выбран.
- Значение с **≈** — оценка пинга: время GET ÷ 3,3. Обычный HTTP GET показывает полное время запроса.
- При включённом системном TUN замер может включать задержку текущего туннеля; его маршруты ради проверки не меняются.
...
- **Nimbo Ping** проверяет серверы подписки по отдельности через HTTP GET, в том числе без подключённого VPN. Текущее подключение не переключается; метод стоит первым и выбран по умолчанию для новых настроек.
- Значение с **≈** — оценка пинга: время GET ÷ 3,3. Обычный HTTP GET показывает полное время запроса.
- Таймаут можно ввести вручную в секундах; результат пинга остаётся в миллисекундах. Исправлена потеря результата при завершении проверки, добавлена краткая диагностика без адресов и ключей.
...
- **Nimbo Ping** проверяет каждый сервер отдельно и работает без подключения VPN. Метод стоит первым и используется по умолчанию для новых настроек; явно выбранный способ сохраняется.
- Значение с **≈** — оценка пинга: время GET ÷ 3,3. Обычный HTTP GET показывает полное время запроса.
- HTTP GET/HEAD через активный VPN и отдельные TCP/ICMP-замеры остаются доступны. При активном VPN на iOS проверка Nimbo Ping может также включать задержку текущего туннеля.
