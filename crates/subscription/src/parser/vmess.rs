...
struct VmessRaw {
    ...
    #[serde(
        ...
        alias = "server_description",
        alias = "server-description",
        alias = "serverdescription"
    )]
    server_description: String,
    #[serde(default, rename = "hostUuid", alias = "host_uuid", alias = "host-uuid")]
    host_uuid: String,
    #[serde(
        default,
        rename = "xrayJsonTemplateUuid",
        alias = "xray_json_template_uuid",
        alias = "xray-json-template-uuid"
    )]
    xray_json_template_uuid: String,
}
...
pub fn parse(input: &str) -> Result<Server, ParseError> {
    ...
    Ok(Server {
        ...
        name,
        server_description: opt(raw.server_description),
        host_uuid: opt(raw.host_uuid),
        xray_json_template_uuid: opt(raw.xray_json_template_uuid),
...
mod tests {
    ...
    fn parses_custom_server_metadata() {
        let json = r#"{"ps":"X","add":"h","port":8443,"id":"u","aid":0,"net":"tcp","tls":"","serverDescription":"Custom desc","hostUuid":"host-1","xrayJsonTemplateUuid":"tpl-1"}"#;
        let url = make_vmess(json);
        let s = parse(&url).unwrap();
