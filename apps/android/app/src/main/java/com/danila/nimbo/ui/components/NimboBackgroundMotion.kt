...
fun DrawScope.drawNimboBackgroundMotion(
    ...
    when (mode) {
        ...
        // ---------- Кольца: расходящиеся окружности ----------
        BackgroundStyleMode.MATERIAL3 -> {
            val center = Offset(w * 0.5f, h * 0.40f)
            repeat(7) { i ->
                val t = wrap01(phase + i / 7f)
                val radius = minDim * (0.10f + t * 1.15f)
                drawCircle(
                    color = c(i).copy(alpha = a * (1f - t) * 1.35f),
                    radius = radius,
                    center = center,
                    style = Stroke(width = minDim * (0.014f - 0.008f * t))
                )
            }
        }

        // ---------- Точки: пульсирующая матрица ----------
        BackgroundStyleMode.NOTHING_DOTS -> {
            val cols = count(10, min = 5)
            val rows = count(20, min = 8)
            val dx = w / cols
            val dy = h / rows
            for (row in 0 until rows) {
                for (col in 0 until cols) {
                    val pulse = 0.5f + 0.5f * sin(phase * TWO_PI + col * 0.55f + row * 0.34f)
                    drawCircle(
                        color = c(col + row).copy(alpha = a * (0.22f + 0.85f * pulse)),
                        radius = dx * 0.12f * (0.65f + 0.65f * pulse),
