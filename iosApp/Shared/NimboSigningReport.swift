...
enum NimboSigningReport {
    ...
    static var problem: String? {
        ...
        guard let app else {
            return "Приложение подписано без профиля Apple. Туннель работать не будет: расширению нужны права из профиля разработчика."
        }
        if app.expired {
        ...
        guard let tunnel else {
            return "В сборке нет подписанного расширения туннеля. Похоже, программа подписи удалила расширения — отключите у неё удаление расширений и подпишите заново."
        }
