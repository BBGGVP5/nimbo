...
struct RootView: View {
    ...
    /// вручную. Проверка только записывает это в диагностику: перезапускать
    /// туннель за человека приложение не берётся — проверочный адрес может
    /// быть недоступен и сам по себе.
    ///
    /// Проверка живёт, пока приложение на экране: в фоне ему работать не дают.
    ///
    ...
    private func startTrafficCheck() {
        ...
        trafficCheck = Task {
            ...
            // Только запись в диагностику. Перезапускать туннель самостоятельно
            // приложение больше не берётся: проверочный адрес может быть
            // недоступен и по чужой причине, а перезапуск на ровном месте
            // оставлял человека вовсе без соединения.
            ...
            await NimboDiagnostics.shared.record(
                ...
                message: "Туннель поднят, но проверочный запрос не прошёл"
            )
        }
    }
