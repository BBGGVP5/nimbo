...
Nimbo is a Windows-first VPN client for xray-compatible subscriptions: Remnawave, Marzban, 3x-ui and any panel that returns standard `vless://`, `vmess://`, `trojan://`, `ss://` or `hysteria2://` links.
...
Nimbo — Windows-first VPN-клиент для xray-совместимых подписок: Remnawave, Marzban, 3x-ui и любых панелей со стандартными ссылками `vless://`, `vmess://`, `trojan://`, `ss://`, `hysteria2://`.
...
- Tunnel log viewer
- Compact responsive layout for mobile-sized previews
...
| **Android** | Planned/mobile preview. The current codebase is desktop-first. |

---

## 📦 Download / Скачать

Download the latest Windows installer from [Releases](../../releases).

Скачайте последний Windows-установщик на странице [Releases](../../releases).
...
```bash
git clone https://github.com/Case211/nimbo-app.git
cd nimbo/apps/ui
npm install
npm run dev
```
...
- Android client or mobile shell
- QR-code subscription import
- Kill switch through Windows Filtering Platform
