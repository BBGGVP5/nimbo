...
if ($expectedSha256 -notmatch '^[0-9a-f]{64}$') {
    throw "GitHub не отдал корректную SHA-256 сумму для libxray-android.zip."
}
...
try {
    ...
    if ($actualSha256 -ne $expectedSha256) {
        throw "Контрольная сумма libXray не совпала: ожидалось $expectedSha256, получено $actualSha256."
    }
