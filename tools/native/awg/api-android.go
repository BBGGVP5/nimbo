...
func awgTurnOn(interfaceName string, tunFd int32, settings string) int32 {
	...
	// Java owns the original ParcelFileDescriptor throughout the tunnel lifetime.
	ownedFd, err := unix.Dup(int(tunFd))
	if err != nil {
		return -1
	}
	tunFd = int32(ownedFd)
	...
	tun, _, err := tun.CreateUnmonitoredTUNFromFD(int(tunFd))
	if err != nil {
	...
	logger.Verbosef("Attaching to VPN interface")
	...
	// Nimbo uses direct JNI; no filesystem UAPI socket or unauthenticated listener.
