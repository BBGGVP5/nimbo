...
class VpnQuickSettingsTileService : TileService() {
    ...
    // Observe Compose state only while the system is displaying the tile.
    private fun startStateObserver() {
        observer?.cancel()
        observer = scope.launch {
            snapshotFlow { listOf(VpnManager.state.value, VpnManager.connectedServer.value?.name,
                VpnManager.recoveryStatus.value, VpnManager.lastConnectionError.value) }.collect {
                updateTileState()
            }
        }
    }
    ...
    override fun onClick() {
        super.onClick()
        if (isLocked) { unlockAndRun { performToggle() }; return }
        performToggle()
    }
    ...
    private fun openApp() {
        try {
            val launchIntent = Intent(this, com.danila.nimbo.QuickControlsActivity::class.java)
            launchIntent.let {
    ...
    private fun updateTileState() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            val en = preferencesManager.appLanguage == "en"
            tile.subtitle = when {
                VpnManager.recoveryStatus.value == com.danila.nimbo.vpn.VpnRecoveryStatus.WAITING_FOR_NETWORK -> if(en) "Waiting for network" else "Ожидание сети"
                VpnManager.lastConnectionError.value != null && state == VpnState.DISCONNECTED -> if(en) "Connection failed · hold to open" else "Ошибка · удерживайте для выбора"
                state == VpnState.CONNECTED -> if(en) "Connected" else "Подключено"
                state == VpnState.CONNECTING -> if(en) "Connecting" else "Подключение"
                else -> if(en) "Disconnected" else "Отключено"
            }
        }
