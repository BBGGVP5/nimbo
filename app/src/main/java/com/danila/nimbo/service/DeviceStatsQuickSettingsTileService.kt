...
class DeviceStatsQuickSettingsTileService : TileService() {
    ...
    private fun updateTileState() {
        ...
        qsTile.label = "Устройства"

        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            qsTile.subtitle = if (stats == null || stats.deviceLimit <= 0) {
                "Лимит не найден"
            } else {
                "${stats.connectedDevicesOnThisPhone}/${stats.deviceLimit}"
            }
        }
