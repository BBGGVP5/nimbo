...
class MainActivity : ComponentActivity() {
    ...
    private var pendingAutoCandidates: List<String> = emptyList()
    ...
    private val vpnPermissionLauncher = registerForActivityResult(
        ...
        if (result.resultCode == RESULT_OK) {
            ...
            VpnManager.autoSelection.value = com.danila.nimbo.vpn.AutoSelectionProgress()
        }
        pendingVpnServer = null
        pendingAutoCandidates = emptyList()
    }
            ...
            CompositionLocalProvider(
                ...
                NebulaGuardTheme(
                    ...
                    MainScreen(
                        ...
                        onAutoConnect = { servers ->
                            val candidates = com.danila.nimbo.vpn.AutoSelectionPolicy.order(servers, false)
                            candidates.firstOrNull()?.let { first ->
                                connectWithPermission(first, viewModel, candidates.map { com.danila.nimbo.vpn.AutoSelectionPolicy.candidateKey(it) })
                            } ?: viewModel.showTopNotification(
                                if (preferencesManager.appLanguage == "en") "No servers available for Auto" else "Нет серверов для автоподбора"
                            )
                        },
    ...
    private fun connectWithPermission(server: Server, viewModel: MainViewModel, autoCandidates: List<String> = emptyList()) {
        ...
        pendingAutoCandidates = autoCandidates
    ...
    private fun startVpnService(server: Server) {
        ...
        if (pendingAutoCandidates.isNotEmpty()) {
            intent.putStringArrayListExtra(MyVpnService.EXTRA_AUTO_CANDIDATES, ArrayList(pendingAutoCandidates))
        }
