...
class MyVpnService : VpnService() {
    ...
    companion object {
        ...
        const val EXTRA_AUTO_CANDIDATES = "auto_candidate_keys"
    ...
    private var autoCandidateKeys: List<String> = emptyList()
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        when (action) {
            ACTION_CONNECT -> {
                autoSelectionGeneration++
                autoCandidateKeys = intent?.getStringArrayListExtra(EXTRA_AUTO_CANDIDATES).orEmpty()
                VpnManager.autoSelection.value = AutoSelectionProgress(
                    running = autoCandidateKeys.isNotEmpty(), total = autoCandidateKeys.size
                )
                ...
                when {
                    ...
                    autoCandidateKeys.isEmpty() && previousServer != null &&
    ...
    private fun connect(currentServer: Server?, recoveryMode: Boolean = false) {
        ...
        connectionJob = serviceScope.launch {
            val autoGeneration = autoSelectionGeneration
            val requestedAutoKeys = autoCandidateKeys.toSet()
            val automatic = requestedAutoKeys.isNotEmpty()
            try {
                // Проверяем интернет
                if (!waitForInternet(recoveryMode)) {
                    Log.e(TAG, "No internet connection")
                    Logger.e(TAG, "Отсутствие подключения к интернету")
                    handleConnectionFailure("No internet connection", retryable = !automatic)
                    return@launch
                }
                ...
                val networkRestricted = if (automatic ||
                    ...
                    isNetworkRestrictedForBypass(automatic)
                } else {
                    false
                }
                ...
                // Auto tests candidates in this same serialized connection cycle.
                // It never starts a second core just to measure a server's port.
                val orderedCandidates = if (automatic) {
                    AutoSelectionPolicy.order(
                        preferencesManager.loadProfiles().flatMap { it.servers }
                            .filter { AutoSelectionPolicy.candidateKey(it) in requestedAutoKeys },
                        networkRestricted
                    )
                } else candidates
                ...
                for ((index, candidate) in orderedCandidates.withIndex()) {
                    ...
                    if (autoGeneration != autoSelectionGeneration) return@launch
                    if (automatic) {
                        VpnManager.autoSelection.value = AutoSelectionProgress(
                            running = true, checked = index, total = orderedCandidates.size,
                    ...
                    if (ConnectionAttemptPolicy.isCoolingDown(
                            ...
                            nowMs = nowMs,
                            explicitSelection = explicitSelection || automatic
                        )
                    ) {
                    ...
                    refreshForegroundNotification()
                    val candidateDeadline = if (automatic) minOf(
                        cycleDeadlineMs, SystemClock.elapsedRealtime() + AutoSelectionPolicy.CANDIDATE_BUDGET_MS
                    ) else cycleDeadlineMs
                    if (automatic) disconnectCurrentEngine()
                    connected = connectCandidate(
                        candidate,
                        cycleDeadlineMs = candidateDeadline,
                        maxAttemptsOverride = if (automatic) 1 else null,
                        probeMode = automatic,
                        recoveryMode = recoveryMode
                    )
                    if (automatic && connected) {
                        connected = withTimeoutOrNull(
                            (candidateDeadline - SystemClock.elapsedRealtime()).coerceAtLeast(1L)
                        ) {
                            if (candidate.usesAwgEngine()) {
                                verifyAwgTunnel(candidate, verifyHandshake = true)
                            } else {
                                val latencies = AutoSelectionProbe.check()
                                val usable = AutoSelectionPolicy.isUsable(latencies)
                                // Единственная честная цифра по узлу: столько заняли
                                // запросы к внешним сервисам, прошедшие сквозь него.
                                // Провал пишем тоже — «узел мёртв» и есть то знание,
                                // которого TCP-пинг не даёт в принципе.
                                VerifiedLatencyStore.record(
                                    measurementKey = candidate.pingMeasurementKey(),
                                    latencyMs = AutoSelectionPolicy.representativeLatency(latencies),
                                    ok = usable
                                )
                                usable && XrayManager.isConnected && hasUsableUnderlyingNetwork()
                            }
                        } ?: false
                        if (!connected) {
                            XrayManager.recordConnectionFailure("Auto selection: HTTPS checks failed")
                    ...
                    if (automatic) {
                        ...
                        if (autoGeneration != autoSelectionGeneration) return@launch
                        VpnManager.autoSelection.value = AutoSelectionProgress(
                            running = !connected, checked = index + 1, total = orderedCandidates.size,
                            serverName = candidate.name, confirmed = connected
                        )
                    }
                    if (connected) {
                ...
                if (!connected) {
                    ...
                    handleConnectionFailure(rawError ?: "Connection failed", retryable = !automatic)
                    return@launch
                }
                ...
                handleConnectionFailure(e.message ?: e::class.java.simpleName, retryable = !automatic)
            } finally {
                if (automatic && autoGeneration == autoSelectionGeneration && VpnManager.autoSelection.value.running) {
                    VpnManager.autoSelection.value = VpnManager.autoSelection.value.copy(running = false, failed = true)
                }
            }
        }
    }
    ...
    private fun requestManualStop(stopReason: String) {
        autoSelectionGeneration++
        autoCandidateKeys = emptyList()
        if (stopReason == "connection failed") {
            if (VpnManager.autoSelection.value.running) {
                VpnManager.autoSelection.value = VpnManager.autoSelection.value.copy(running = false, failed = true)
            }
        } else {
            VpnManager.autoSelection.value = AutoSelectionProgress()
    ...
    private fun handleConnectionFailure(message: String, retryable: Boolean) {
        ...
        if (autoCandidateKeys.isNotEmpty() && !retryable) {
            Logger.w(TAG, "Auto selection finished without a verified connection")
            VpnManager.autoSelection.value = VpnManager.autoSelection.value.copy(running = false, failed = true)
    ...
    private suspend fun probeThroughSelectedOutbound(
        ...
        timeoutMs: Int = TunnelHealthPolicy.REQUEST_TIMEOUT_MS
    ): List<Int> = coroutineScope {
        targets.map { target ->
            async { probeTargetThroughSelectedOutbound(target, timeoutMs) }
        }.awaitAll()
    }

    private suspend fun probeTargetThroughSelectedOutbound(target: TunnelProbeTarget, timeoutMs: Int = TunnelHealthPolicy.REQUEST_TIMEOUT_MS): Int =
        try {
    ...
    private suspend fun isNetworkRestrictedForBypass(automatic: Boolean = false): Boolean {
        if (!automatic && !preferencesManager.autoBypassByNetwork) return false
        ...
        val restricted = ConnectionAttemptPolicy.isRestricted(baseline, targetServices) ||
            (automatic && targetServices.isNotEmpty() && targetServices.none { it })
        Log.i(
            TAG,
