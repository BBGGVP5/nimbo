...
/**
 * Панель вместо серверов иногда отдаёт «уведомление»: записи-заглушки, у которых
 * адрес — петля или пустой хост, порт нерабочий, а весь смысл лежит в имени
 * («Включите HWID в настройках», «Обратитесь в поддержку»). Формально это
 * валидные ссылки, поэтому такой ответ выглядит непустым — и способен затереть
 * рабочий список серверов.
 *
 * Узел на 127.0.0.1 или на порту 0/1 не бывает настоящим: там нечему отвечать.
 */
fun isNoticePlaceholderServer(server: Server): Boolean {
    val host = server.host.trim().lowercase().removeSurrounding("[", "]")
    val loopbackOrUnspecified = host.isBlank() ||
        host == "localhost" ||
        host == "::1" ||
        host == "0.0.0.0" ||
        host == "::" ||
        host.startsWith("127.")
    if (loopbackOrUnspecified) return true

    // Порт 0/1 не слушает ни один реальный узел.
    if (server.port <= 1) return true

    // Нулевой идентификатор — верный признак записи-пустышки.
    val uuid = server.uuid.trim().lowercase()
    return uuid.isNotBlank() && uuid.all { it == '0' || it == '-' } && uuid.any { it == '0' }
}
