...
object SubscriptionManager {
    ...
    private fun dedupeKeyForLink(link: String): String {
        ...
        return if (isHysteriaProtocolLink(trimmed)) {
            // Ключ hysteria намеренно не смотрит на query-мусор, но имя учитывать
            // обязан: одна точка входа может быть несколькими пунктами подписки
            // («Обходы LTE», «АВТО», «Обход 1»), и склеивать их нельзя.
            val name = shareLinkFragmentName(trimmed).trim()
            val nameKey = if (isGenericServerName(name)) "" else name.lowercase()
            "hysteria|${canonicalHysteriaLinkKey(trimmed)}|$nameKey"
        } else {
            "link|$trimmed"
        }
    }
    ...
    private fun dedupeServerLinks(links: List<String>): List<String> {
        ...
        links
            ...
            .forEach { link ->
                byKey.putIfAbsent(dedupeKeyForLink(link), link)
            }

        // Безымянный hysteria-узел — это тот же сервер, просто пришедший из
        // источника без имени: убираем его, если та же точка входа уже есть под
        // нормальным названием. Именованные записи при этом не склеиваются между
        // собой, даже когда делят одну точку входа: в подписке это разные пункты.
        val namedHysteriaEndpoints = byKey.values
            .asSequence()
            .filter { isHysteriaProtocolLink(it) && !isGenericShareLinkName(it) }
            .map { canonicalHysteriaLinkKey(it) }
            .toSet()

        return byKey.values.filterNot { link ->
            isHysteriaProtocolLink(link) &&
                isGenericShareLinkName(link) &&
                canonicalHysteriaLinkKey(link) in namedHysteriaEndpoints
        }
    }
    ...
    internal fun parseServerLinksFromClientJsonConfig(jsonText: String): List<String> {
        ...
        return runCatching {
            ...
            // Формат Happ/Remnawave «массив конфигов»: каждый элемент — это
            // отдельный пункт списка серверов, а не набор серверов.
            //
            // Схлопывать их обычным dedupeServerLinks нельзя: его ключ для
            // hysteria намеренно игнорирует #fragment, и тогда разные пункты
            // подписки, которые делят одну точку входа, слипаются в один.
            // Здесь список задан панелью, поэтому убираем только точные повторы.
            namedClientConfigArrayLinks(root)?.let { return@runCatching it.distinct() }

            val outbounds = collectOutboundsArrays(root)
            val inbounds = collectInboundsArrays(root)

            dedupeServerLinks(
                outbounds.flatMap { array -> outboundShareLinks(array) } +
    ...
    /** Замена #fragment у собранной ссылки без android.net.Uri. */
    private fun renameShareLink(link: String, name: String): String {
        val base = link.substringBefore('#')
        return "$base#${encodeUriComponent(name)}"
    }

    private fun outboundShareLinks(array: JSONArray): List<String> = buildList {
        for (i in 0 until array.length()) {
            val outbound = array.optJSONObject(i) ?: continue
            val link = xrayVlessLinkFromOutbound(outbound)
                ?: xrayTrojanLinkFromOutbound(outbound)
    ...
    /**
     * Подписки Happ-формата отдают корневой JSON-массив, где каждый элемент —
     * это законченный клиентский конфиг и одновременно один пункт списка
     * серверов; его человекочитаемое имя лежит в поле "remarks".
     *
     * Разворачивать outbounds такого элемента в отдельные серверы нельзя: у
     * записи с балансировщиком это участники одной и той же локации
     * (`bal`, `bal-2`, `bal-3`…), а у обычной записи — единственный outbound с
     * техническим тегом "proxy". И то и другое даёт список из технических имён
     * вместо названий вроде «🇫🇮 Финляндия 1».
     *
     * Возвращает null, если payload не в этом формате — тогда работает прежний
     * разбор через [collectOutboundsArrays].
     */
    private fun namedClientConfigArrayLinks(root: Any?): List<String>? {
        val array = root as? JSONArray ?: return null
        if (array.length() == 0) return null

        val configs = ArrayList<JSONObject>(array.length())
        for (i in 0 until array.length()) {
            val config = array.optJSONObject(i) ?: return null
            if (config.optJSONArray("outbounds") == null) return null
            configs.add(config)
        }
        // Хотя бы один элемент должен быть подписан, иначе это обычный массив
        // конфигов без имён и старый разбор ничем не хуже.
        if (configs.none { !it.cleanString("remarks", "remark", "title").isNullOrBlank() }) return null

        return buildList {
            for (config in configs) {
                val outbounds = config.optJSONArray("outbounds") ?: continue
                val links = outboundShareLinks(outbounds)
                val remarks = config.cleanString("remarks", "remark", "title")
                if (remarks.isNullOrBlank()) {
                    addAll(links)
                } else {
                    // Все outbound'ы конфига ведут в одну и ту же точку списка,
                    // поэтому берём первый рабочий и подписываем его именем конфига.
                    links.firstOrNull()?.let { add(renameShareLink(it, remarks)) }
                }
            }
        }
    }
    ...
    private fun xrayVlessLinkFromOutbound(outbound: JSONObject): String? {
        ...
        params.putAll(streamSettingsShareParams(stream))
        params["flow"] = user.cleanString("flow")
    ...
    /**
     * Normalizes an Xray trojan client outbound into a trojan share link.
     * Без этого конфиги, у которых trojan — единственный proxy-outbound,
     * не давали вообще никакой ссылки и сервер пропадал из списка.
     */
    private fun xrayTrojanLinkFromOutbound(outbound: JSONObject): String? {
        if (!outbound.cleanString("protocol").orEmpty().equals("trojan", ignoreCase = true)) return null

        val node = outbound.optJSONObject("settings")
            ?.optJSONArray("servers")
            ?.optJSONObject(0)
            ?: return null
        val host = node.cleanString("address", "host", "server") ?: return null
        val port = node.cleanInt("port", "server_port", "serverPort") ?: return null
        val password = node.cleanString("password", "auth") ?: return null

        val stream = outbound.optJSONObject("streamSettings") ?: JSONObject()
        val query = streamSettingsShareParams(stream)
            .filterValues { !it.isNullOrBlank() }
            .entries
            .joinToString("&") { (key, value) ->
                "${encodeUriComponent(key)}=${encodeUriComponent(value.orEmpty())}"
        ...
        val safeHost = if (host.contains(':') && !host.startsWith('[')) "[$host]" else host
        val tag = outbound.cleanString("remarks", "remark", "name", "tag") ?: "Trojan $host"
        return "trojan://${encodeUriComponent(password)}@$safeHost:$port?$query#${encodeUriComponent(tag)}"
    }

    /**
     * Транспортная часть share-ссылки из streamSettings клиентского конфига.
     *
     * Reality хранит sni/отпечаток и ключи в собственном блоке realitySettings,
     * а не в tlsSettings: если читать только tlsSettings, из ссылки пропадают
     * pbk и sid, без которых Reality-сервер не отвечает. Так же теряется
     * serviceName у gRPC и path/Host у WebSocket.
     */
    private fun streamSettingsShareParams(stream: JSONObject): LinkedHashMap<String, String?> {
        ...
        val handshake = reality ?: tls

        return linkedMapOf(
            "type" to (stream.cleanString("network") ?: "tcp"),
            "security" to (stream.cleanString("security") ?: "none"),
            "sni" to handshake?.cleanString("serverName", "server_name", "sni"),
            "fp" to handshake?.cleanString("fingerprint"),
            "alpn" to tls?.cleanStringArray("alpn"),
            "allowInsecure" to tls?.cleanBoolean("allowInsecure")?.toString(),
            "pbk" to reality?.cleanString("publicKey", "public_key"),
            "sid" to reality?.cleanString("shortId", "short_id"),
            "spx" to reality?.cleanString("spiderX", "spider_x"),
            "path" to (xhttp?.cleanString("path") ?: ws?.cleanString("path")),
            ...
            "host" to (
                xhttp?.cleanString("host")
                    ?: ws?.optJSONObject("headers")?.cleanString("Host", "host")
                ),
            "mode" to xhttp?.cleanString("mode"),
            "extra" to xhttp?.optJSONObject("extra")?.toString(),
            "serviceName" to grpc?.cleanString("serviceName", "service_name"),
            "authority" to grpc?.cleanString("authority")
        )
    }
    ...
    internal fun parseServerLinksFromSubscriptionText(text: String): List<String> {
