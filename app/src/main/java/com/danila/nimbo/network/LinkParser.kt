...
object LinkParser {
    ...
    private fun parseTrojan(link: String, uri: Uri?): Server {
        ...
        return Server(
            ...
            // Trojan тоже бывает поверх Reality: без pbk/sid XrayManager соберёт
            // realitySettings без ключей и рукопожатие не пройдёт.
            publicKey = params["pbk"] ?: uri?.getQueryParameter("pbk"),
            shortId = params["sid"] ?: uri?.getQueryParameter("sid"),
            spiderX = params["spx"] ?: uri?.getQueryParameter("spx"),
            templateUuid = templateUuid,
            templateName = templateName
        )
