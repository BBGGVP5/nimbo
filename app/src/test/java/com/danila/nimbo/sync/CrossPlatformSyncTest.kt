...
class CrossPlatformSyncTest {
    ...
    @Test
    fun `bluetooth qr parameters parse correctly`() {
        val parsed = CrossSyncProtocol.parseQr(
            "nimbo-sync://pair?v=1&host=192.168.1.20&port=42000&sid=session-1&key=$encodedKey&exp=$future&bt_mac=AA:BB:CC:DD:EE:FF&transport=bluetooth",
            nowMs = future - 10_000
        )

        assertEquals("AA:BB:CC:DD:EE:FF", parsed.bluetoothMac)
        assertEquals("bluetooth", parsed.preferredTransport)
    }
    ...
    @Test
    fun `discovery beacon session generates valid packet`() {
        val beacon = CrossSyncDiscoveryEngine.generateNewBeaconSession()
        assertEquals("NIMBO_DISCOVERY_V1", beacon.type)
        assertEquals(42000, beacon.port)
        assertEquals(6, beacon.comparisonCode.length)
        assertTrue(beacon.expiresAtMs > System.currentTimeMillis())
        ...
        val peer = DiscoveredPeerDevice(
            ...
            platform = "android",
            host = "192.168.1.50",
            lastSeenMs = System.currentTimeMillis() - 20_000L
        )
        assertTrue(peer.isExpired(System.currentTimeMillis()))
    }
}
