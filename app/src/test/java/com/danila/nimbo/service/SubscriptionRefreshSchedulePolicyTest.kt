...
class SubscriptionRefreshSchedulePolicyTest {
    ...
    fun notificationRequiresChangeAndBackground() {
        ...
        assertFalse(SubscriptionRefreshSchedulePolicy.shouldShowSystemNotification(true, false, 1, vpnActive = true))
    }

    @Test
    fun refreshIsPausedWhileVpnIsActiveOrDesired() {
        assertTrue(SubscriptionRefreshSchedulePolicy.canRefreshSubscriptions(true, false, false))
        assertFalse(SubscriptionRefreshSchedulePolicy.canRefreshSubscriptions(false, false, false))
        assertFalse(SubscriptionRefreshSchedulePolicy.canRefreshSubscriptions(true, true, false))
        assertFalse(SubscriptionRefreshSchedulePolicy.canRefreshSubscriptions(true, false, true))
    }
}
